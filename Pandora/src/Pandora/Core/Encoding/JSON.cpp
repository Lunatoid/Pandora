...
bool JsonValue::TryGetNumber(f64& target) {
...
bool JsonValue::TryGetString(String& target) {
...
bool JsonValue::TryGetArray(JsonValue& target) {
...
bool JsonValue::TryGetObject(JsonValue& target) {
...
bool JsonValue::TryGetBool(int index, bool& target) {
    if (CanEnumerate() && index >= 0 && index < Count()) {
        JsonValue& element = GetElement(index);
        if (element.Type() == JsonType::Bool) {
            target = element.GetBool();
            return true;
        }
    }
...
bool JsonValue::TryGetString(int index, String& target) {
    if (CanEnumerate() && index >= 0 && index < Count()) {
        JsonValue& element = GetElement(index);
...
bool JsonValue::TryGetArray(int index, JsonValue& target) {
    if (CanEnumerate() && index >= 0 && index < Count()) {
        JsonValue& element = GetElement(index);
...
bool JsonValue::TryGetObject(int index, JsonValue& target) {
    if (CanEnumerate() && index >= 0 && index < Count()) {
        JsonValue& element = GetElement(index);
...
bool JsonValue::TryGetNumber(StringView key, f64& target) {
...
bool JsonValue::TryGetBool(StringView key, bool& target) {
    if (Type() == JsonType::Object && HasField(key)) {
        JsonValue& element = GetField(key);
        if (element.Type() == JsonType::Bool) {
            target = element.GetBool();
...
bool JsonValue::TryGetString(StringView key, String& target) {
    if (Type() == JsonType::Object && HasField(key)) {
        JsonValue& element = GetField(key);
        if (element.Type() == JsonType::String) {
            target = element.GetString();
...
bool JsonValue::TryGetArray(StringView key, JsonValue& target) {
    if (Type() == JsonType::Object && HasField(key)) {
        JsonValue& element = GetField(key);
        if (element.Type() == JsonType::Array) {
            target = element;
...
bool JsonValue::TryGetObject(StringView key, JsonValue& target) {
    if (Type() == JsonType::Object && HasField(key)) {
        JsonValue& element = GetField(key);
        if (element.Type() == JsonType::Object) {
            target = element;
...
int JsonValue::GetFieldIndex(StringView key) const {
    PD_ASSERT_D(Type() == JsonType::Object, "JSON type mismatch");
