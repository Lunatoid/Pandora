/* ========================================================================
   ...
   A: Quickly hashing any amount of data for comparison purposes such as
      block deduplication or change detection.  It is fast on all memory
      ...
      or b) can always supply a small number of bytes in a memory padded to some
   ...
   A: The final Meow Hash was created as a collaboration between
      ...
      way to handle the residual end-of-memory bytes that dramatically
...
MeowHash(void* Seed128Init, meow_umm Len, void* SourceInit) {
    ...
    //
    // TODO(casey): I need to put more thought into how the end-of-memory stuff is actually working out here,
    // because I _think_ it may be possible to remove the first branch (on Len8) and let the mask zero out the
    // result, but it would take a little thought to make sure it couldn't read off the end of the memory due
