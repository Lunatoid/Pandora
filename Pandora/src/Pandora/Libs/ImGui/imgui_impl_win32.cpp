#if !defined(PD_NO_IMGUI) && defined(PD_WINDOWS)
...
static void ImGui_ImplWin32_CreateWindow(ImGuiViewport* viewport) {
    ...
    // Here is custom code handling OpenGL/Win32 viewports
    ...
    Handles* handles = (Handles*)ImGui::GetMainViewport()->RendererUserData;

    // Handles will only be non-null if we're using OpenGL
    if (handles) {
        // Get pixel format from the parent window and create a shared context
        PIXELFORMATDESCRIPTOR pfd = {};
        int format = GetPixelFormat(handles->device);
        DescribePixelFormat(handles->device, format, sizeof(pfd), &pfd);

        SetPixelFormat(GetDC(data->Hwnd), format, &pfd);
        ...
        HDC viewportHDC = GetDC((HWND)viewport->PlatformUserData);

        viewport->RendererUserData = wglCreateContextAttribsARB(viewportHDC, handles->context, glVersion);
        wglMakeCurrent(viewportHDC, (HGLRC)viewport->RendererUserData);
    }
}
...
static void ImGui_ImplWin32_DestroyWindow(ImGuiViewport* viewport) {
    ...
    // [CUSTOM]
    wglDeleteContext((HGLRC)viewport->RendererUserData);
